...
//对证名人身份的核对
func (zkp *Zkpi1) ProverCalc(params *PublicParameters,
	eta *big.Int,
	rand *rand.Rand,
	r, c1, c2, c3 *big.Int,
) {
	...
	//v = (c2)α mod N2
	zkp.v = util.ModPowInsecure(c2, alpha, nSquared)

	digest := util.Sha256Hash(util.GetBytes(c1), util.GetBytes(c2), util.GetBytes(c3),
		util.GetBytes(zkp.z), util.GetBytes(zkp.u1), util.GetBytes(zkp.u2), util.GetBytes(zkp.v))
	if len(digest) == 0 {
	...
	//s2 = (r)e×β mod N
	var ren = util.ModPowInsecure(r, zkp.e, N)
...
func (zkp *Zkpi1) checkU1(g, nSquared, N, c3 *big.Int) {
	...
	var z = util.ModPowInsecure(c3, c3neg, nSquared)
...
func (zkp *Zkpi1) checkV(c2, nSquared, c1 *big.Int) {
	var x = util.ModPowInsecure(c2, zkp.s1, nSquared)
	var eneg = new(big.Int).Neg(zkp.e)
	var y = util.ModPowInsecure(c1, eneg, nSquared)
	var mulxy = new(big.Int).Mul(x, y)
...
func (zkp *Zkpi1) checkE(c1, c2, c3 *big.Int) {
	var result = util.Sha256Hash(util.GetBytes(c1), util.GetBytes(c2), util.GetBytes(c3),
		util.GetBytes(zkp.z), util.GetBytes(zkp.u1), util.GetBytes(zkp.u2), util.GetBytes(zkp.v))
