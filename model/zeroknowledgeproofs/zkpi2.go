...
//对签名数据进行核对
func (zkp *Zkpi2) ProverCalc(params *PublicParameters,
	eta1, eta2 *big.Int,
	rand *rand.Rand,
//c *ECPoint,
	cx, cy,
	...
	//δ ∈ (Z)q3
	//var delta= crypto.util.RandomFromZn(crypto.Pow(q, 3))
	//μ ∈ (Z) N
	var mu = util.RandomFromZnStar(N)
	//ν ∈ (Z)q3 * Ñ
	//var q3=crypto.Pow(q, 3)
	//var nu=crypto.util.RandomFromZn(new(big.Int).Mul(q3,nTilde))
	//θ ∈ (Z)q8
	var q6 = new(big.Int).Mul(q3, q3)
	var q8 = new(big.Int).Mul(q6, q2)
	var theta = util.RandomFromZn(q8)
	//τ ∈ (Z)q8 * Ñ
	var tau = util.RandomFromZn(new(big.Int).Mul(q8, nTilde))
	//ρ1 ∈ (Z)qÑ
	var rho1 = util.RandomFromZn(new(big.Int).Mul(q, nTilde))
	//ρ 2 ∈ (Z)q6 * Ñ
	var rho2 = util.RandomFromZn(new(big.Int).Mul(q6, nTilde))
	//z1
	hen := util.ModPowInsecure(h1, eta1, nTilde)
	hrn := util.ModPowInsecure(h2, rho1, nTilde)
	hh := new(big.Int).Mul(hen, hrn)
	zkp.z1 = new(big.Int).Mod(hh, nTilde)
	//z2
	hhen := util.ModPowInsecure(h1, eta2, nTilde)
	hhrn := util.ModPowInsecure(h2, rho2, nTilde)
	...
	//u2 = (Γ)α * (β)N mod N 2
	var gan = util.ModPowInsecure(g, alpha, nSquared)
	var bnn = util.ModPowInsecure(beta, N, nSquared)
	var gb = new(big.Int).Mul(gan, bnn)
	zkp.u2 = new(big.Int).Mod(gb, nSquared)
	//u3 = (h1)α * (h2)γ mod Ñ
	var han = util.ModPowInsecure(h1, alpha, nTilde)
	var hgn = util.ModPowInsecure(h2, gamma, nTilde)
	var hhag = new(big.Int).Mul(han, hgn)
	zkp.u3 = new(big.Int).Mod(hhag, nTilde)
	//v1 = (u)α (Γ)qθ * (μ)N mod N2
	var uan = util.ModPowInsecure(u, alpha, nSquared)
	var qtta = new(big.Int).Mul(q, theta)
	var gqn = util.ModPowInsecure(g, qtta, nSquared)
	var mnn = util.ModPowInsecure(mu, N, nSquared)
	var ug = new(big.Int).Mul(uan, gqn)
	var um = new(big.Int).Mul(ug, mnn)
	zkp.v1 = new(big.Int).Mod(um, nSquared)
	//v3 = (h1)θ * (h2)τ mod Ñ
	var hthn = util.ModPowInsecure(h1, theta, nTilde)
	var htnt = util.ModPowInsecure(h2, tau, nTilde)
	var aaa = new(big.Int).Mul(hthn, htnt)
	zkp.v3 = new(big.Int).Mod(aaa, nTilde)
	//e = hash(g, w, u, z1 , z2 , u1 , u2 , u3 , v1 , v2 , v3)
	digest := util.Sha256Hash(util.GetBytes(g), util.GetBytes(w), util.GetBytes(u), util.GetBytes(zkp.z1),
		util.GetBytes(zkp.z2), util.GetBytes(zkp.u1), util.GetBytes(zkp.u2), util.GetBytes(zkp.u3),
		util.GetBytes(zkp.v1), util.GetBytes(zkp.v3))
	if len(digest) == 0 {
	...
	rande := util.ModPowInsecure(randomness, zkp.e, N)
...
func (zkp *Zkpi2) checkU1(cx, cy *big.Int, rx, ry *big.Int) {
	...
	x1 := new(big.Int).Mul(new(big.Int).SetBytes(util.Get2Bytes(cx, cx)), zkp.s1)
	var nege = new(big.Int).Neg(zkp.e)
	x2 := new(big.Int).Mul(new(big.Int).SetBytes(util.Get2Bytes(rx, rx)), nege)
...
func (zkp *Zkpi2) checkU3(h1, nTilde, h2 *big.Int) {
	hsn := util.ModPowInsecure(h1, zkp.s1, nTilde)
	hsnt := util.ModPowInsecure(h2, zkp.s2, nTilde)
	en := new(big.Int).Neg(zkp.e)
	hhss := new(big.Int).Mul(hsn, hsnt)
	zenn := util.ModPowInsecure(zkp.z1, en, nTilde)
...
func (zkp *Zkpi2) checkV1(u *big.Int, nSquared *big.Int, q *big.Int, g *big.Int, N *big.Int, w *big.Int) {
	usn := util.ModPowInsecure(u, zkp.s1, nSquared)
	qt := new(big.Int).Mul(q, zkp.t2)
	gq := util.ModPowInsecure(g, qt, nSquared)
	tnn := util.ModPowInsecure(zkp.t1, N, nSquared)
	en := new(big.Int).Neg(zkp.e)
	wen := util.ModPowInsecure(w, en, nSquared)
...
func (zkp *Zkpi2) checkV3(h1 *big.Int, nTilde *big.Int, h2 *big.Int) {
	h1tn := util.ModPowInsecure(h1, zkp.t2, nTilde)
	htn := util.ModPowInsecure(h2, zkp.t3, nTilde)
	en := new(big.Int).Neg(zkp.e)
	zen := util.ModPowInsecure(zkp.z2, en, nTilde)
...
func (zkp *Zkpi2) checkE(bitC *ECPoint, w, u, g *big.Int) { //util.Get2Bytes(bitC.X,bitC.Y)
	var result = util.Sha256Hash(util.GetBytes(g), util.GetBytes(w), util.GetBytes(u),
		util.GetBytes(zkp.z1), util.GetBytes(zkp.z2),
		util.GetBytes(zkp.u1), util.GetBytes(zkp.u2), util.GetBytes(zkp.u3),
		util.GetBytes(zkp.v1), util.GetBytes(zkp.v3))
